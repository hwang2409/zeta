...
impl MessageScrollerState {
    ...
    ///
    /// `ListAlignment::Bottom` rests short content on the bottom edge of the
    /// viewport (chat-UI convention); `ListAlignment::Top` matches the
    /// default `new` constructor. Tail following is enabled regardless, so
    /// live streaming continues to pin the newest row into view.
