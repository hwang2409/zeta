...
from ._attachments import (
    ...
    ATTACHMENT_TOKEN_RE,  # noqa: F401 - preserve the composer import
    AttachmentError,
    AttachmentRef,  # noqa: F401 - preserve the composer import
    _attachment_path,
    _image_media_type,  # noqa: F401 - preserve the composer import
    _read_attachment,  # noqa: F401 - preserve the composer import
    attachment_refs,  # noqa: F401 - preserve the composer import
    build_user_message,
    paste_image,
)
...
class TurnConsumerMixin:
    ...
    async def _consume_turn(
        ...
        *,
        ...
        turn_abort_signal = (
            abort_signal
            or self.loop.tool_registry.abort_signal.registry.new_generation()
        )
        ...
        try:
            ...
            async for event in events:
                ...
                if self.verbose:
                    self._print(
                        Text(
                            json.dumps(event.to_dict(), sort_keys=True), style=theme.DIM
                        )
                    )
                    ...
                    if rendered is not None:
                        ...
                        if event.type in {
                            ...
                            if (
                                ...
                                and is_retryable_error(event.error)
                            ):
                                turn_failed = True
...
class ComposerAttachmentMixin:
    ...
    def slash_paste(self, args: str) -> str:
        ...
        try:
            path = paste_image(
                self.loop.store.session_dir, directory_fd=self.loop.store.directory_fd
            )
    ...
    def _pending_paths_for(
        ...
        *,
        ...
        token_paths = sorted(
            ((value.index(token), path) for token, path in tokens.items()),
            key=lambda item: item[0],
        )
        selected_paths = [path for _, path in token_paths]
        selected_paths.extend(
            path for path in attachments if path not in remaining_mapped_paths
        )
    ...
    def _undo_candidate_for_message(self, text: str, message: Message) -> UndoCandidate:
