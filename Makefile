...
# reinstating the upstream `Definite(...)` shape that drops the last glyph
# of any 9-char code chip. The pixel-gutter guard must panic under this
# mutation. A green run here means the fix silently reverted (a rebase
# picked the wrong side, a peer refactor threw the env branch away) and
# the ladder guard would ship broken.
...
gui-native-guards-inline-flow-mutation:
	...
	if ZETA_HOME="$$tmp_dir/home" ZETA_GUI_NATIVE_GUARDS=1 ZETA_GUI_INLINE_FLOW_DEFINITE=1 \
	  cargo run --manifest-path gui/Cargo.toml --features smoke-test -- --socket "$$socket"; then \
		echo "FAIL: guard mutation PASSED with ZETA_GUI_INLINE_FLOW_DEFINITE=1 (fix silenced)"; \
		exit 1; \
	else \
